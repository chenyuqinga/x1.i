...
public class WheelView extends View {
    ...
    Paint paintlabelText;
    Paint paintIndicator;

    WheelAdapter adapter;

    private String labelLeft = "";//附加单位
    private String labelRight = "";//附加单位
    private int leftLabelPadding = 0;
    private int rightLabelPadding = 0;
    int textSize;//选项的文字大小
    int outTextSize;
    int labelSize;//单位文字大小
    ...
    private boolean isGlobalCenter = true;
    /**
     * 是否显示间隔的横线
     */
    ...
    private int moveLabelRangeX = 0;
    private int moveLabelRangeY = 0;

    private int mGravity = Gravity.CENTER;
    private int drawCenterContentStart = 0;//中间选中文字开始绘制位置
    private int drawLableLeftStart = 0;//非中间文字开始绘制位置
    private int drawLableRightStart = 0;//单位开始绘制位置
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        outTextSize = getResources().getDimensionPixelSize(R.dimen.pickerview_outtextsize);
        labelSize = getResources().getDimensionPixelSize(R.dimen.pickerview_labelsize);
        ...
        if (attrs != null) {
            ...
            outTextSize = a.getDimensionPixelOffset(R.styleable.wheelview_textOutSize, outTextSize);
            labelSize = a.getDimensionPixelOffset(R.styleable.wheelview_labelSize, outTextSize);
        }
        leftLabelPadding = getResources().getDimensionPixelSize(R.dimen.pickerview_lable_padding);
        rightLabelPadding = getResources().getDimensionPixelSize(R.dimen.pickerview_lable_padding);
    ...
    public final void setTextOutSize(float size) {
        if (size > 0.0F && !customTextSize) {
            outTextSize = (int) (context.getResources().getDisplayMetrics().density * size);
            paintOuterText.setTextSize(outTextSize);
        }
    }
    ...
    public int setItemByDate(int currentDate) {
        if (adapter != null && adapter.getData() != null && adapter.getData().size() > 0) {
            for (int i = 0; i < adapter.getData().size(); i++) {
                int data = Integer.parseInt((String) adapter.getData().get(i));
                if (data == currentDate) {
                    setCurrentItem(i);
                    return i;
                }
    ...
    public final void setAdapter(WheelAdapter adapter,boolean isCyclic) {
        this.adapter = adapter;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight, paintOuterText);
                    canvas.restore();
                    canvas.save();
        ...
        if (!TextUtils.isEmpty(labelLeft)) {
            canvas.drawText(labelLeft, drawLableLeftStart, centerY + moveLabelRangeY, paintlabelText);
        }

        if (!TextUtils.isEmpty(labelRight)) {
            canvas.drawText(labelRight, drawLableRightStart, centerY + moveLabelRangeY, paintlabelText);
        }
    }
    ...
    private void measuredLabelContentStart() {
        Rect rectRight = new Rect();
        paintCenterText.getTextBounds(labelRight, 0, labelRight.length(), rectRight);

        Rect rectLeft = new Rect();
        paintCenterText.getTextBounds(labelLeft, 0, labelLeft.length(), rectLeft);
        switch (mGravity) {
            case Gravity.CENTER:
                if (!isGlobalCenter) {
                    drawCenterContentStart = (int) ((measuredWidth) * 0.5) + moveRange;
                } else {
                    drawCenterContentStart = (measuredWidth - rectRight.width() / 2 + rectLeft.width() / 2 - maxTextWidth / 2 + leftLabelPadding / 2 - rightLabelPadding / 2) / 2 + moveRange;
                }
                drawLableRightStart = drawCenterContentStart + maxTextWidth / 2 + rectRight.width() / 2 + rightLabelPadding + moveLabelRangeX;
                drawLableLeftStart = drawCenterContentStart - maxTextWidth / 2 - rectLeft.width() / 2 - leftLabelPadding;
                break;
            case Gravity.LEFT:
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (adapter.getItemsCount() == 1 && !isLoop) {
            return true;
        }
     ...
     * 附加在右边的单位字符串
     */
    public void setLabel(String labelLeft) {
    ...
    public void setLeftLabelPadding(int leftLabelPadding) {
        this.leftLabelPadding = dip2px(leftLabelPadding);
    }

    public void setRightLabelPadding(int rightLabelPadding) {
        this.rightLabelPadding = dip2px(rightLabelPadding);
    }
    ...
    public void setMoveRange(int moveRange) {
        this.moveRange = dip2px(moveRange);
        invalidate();
    }
     ...
     * 负 ： 向左偏移， 正向右偏移
     *
     * @param moveLabelRange
     */
    public void setMoveLabelRangeX(int moveLabelRange) {
        this.moveLabelRangeX = dip2px(moveLabelRange);
        invalidate();
    }
     ...
     * @param moveLabelRange
     */
    public void setMoveLabelRangeY(int moveLabelRange) {
        this.moveLabelRangeY = dip2px(moveLabelRange);
        invalidate();
    }


    public void setGlobalCenter(boolean globalCenter) {
        isGlobalCenter = globalCenter;
        measuredLabelContentStart();
        invalidate();
    }

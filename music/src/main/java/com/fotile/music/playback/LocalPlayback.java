...
public final class LocalPlayback implements Playback {
    ...
    public LocalPlayback(Context context, MusicProvider musicProvider) {
        Log.e(TAG, "LocalPlayback constructor");
        Context applicationContext = context.getApplicationContext();
        this.mContext = applicationContext;
        this.mMusicProvider = musicProvider;
        this.mAudioManager = (AudioManager) applicationContext.getSystemService(
                Context.AUDIO_SERVICE);
        Log.e(TAG, "mAudioManager: " + mAudioManager);
        // Create the Wifi lock (this does not acquire the lock, this just creates it)
        this.mWifiLock = ((WifiManager) applicationContext.getSystemService(Context.WIFI_SERVICE))
                .createWifiLock(WifiManager.WIFI_MODE_FULL, "uAmp_lock");
        Log.e(TAG, "mWifiLock: " + mWifiLock);
    }
    ...
    public void play(QueueItem item) {
        ...
        if (mediaHasChanged || mExoPlayer == null) {
            ...
            // tracks.
            final AudioAttributes audioAttributes = new AudioAttributes.Builder().setContentType(
                    CONTENT_TYPE_MUSIC).setUsage(USAGE_MEDIA).build();
    ...
    private final class ExoPlayerEventListener implements ExoPlayer.EventListener {
        ...
        public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
            switch (playbackState) {
                ...
                case ExoPlayer.STATE_READY:
                    if (mCallback != null) {
                        Log.e(TAG, "duration: " + mExoPlayer.getDuration());
                        Log.e(TAG,
                                "state: " + playbackState + ", playWhenReady: " + playWhenReady);
                        mCallback.onPlaybackStatusChanged(getState());
                        long duration = mExoPlayer.getDuration();
                        int audioSessionId = mExoPlayer.getAudioSessionId();
                        Log.e(TAG, "audio session id: " + audioSessionId);
                        if (duration > 0 && audioSessionId == 0) {
                            mCallback.onDurationGet(duration);
                        }
                    }
                    break;
                case ExoPlayer.STATE_ENDED:

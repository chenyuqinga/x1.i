/*
 * ************************************************************
 * 文件：VariableLenStickPackageHelper.java  模块：voice  项目：X1.I
 * 作者：coiliaspp
...
public class VariableLenStickPackageHelper implements AbsStickPackageHelper {
    public static final String TAG = VariableLenStickPackageHelper.class.getSimpleName();
    ...
    private List<Byte> mExtraBytes;
    private TcpMsgStickyResultListener mListener;

    public VariableLenStickPackageHelper(ByteOrder byteOrder, int lenSize, int lenIndex,
            int offset) {
    ...
    public List<Byte> execute(InputStream is) {
        mBytes.clear();
        int count = 0;
        int len = -1;
        byte temp;
...
//                        mBytes.add(temp);
                        Log.e(TAG, "enough: " + mBytes.size());
                        mIsEnough = true;
                        if (mListener != null) {
                            mListener.onSnippetGet(mBytes);
                        }
//                        break;
                    } else if (count > msgLen + offset) {//error
//                        Log.e(TAG, "more: " + mBytes.size());
//                        len = -1;//标记为error
//                        mExtraBytes.add(temp);
                        Log.e(TAG, "more: " + mExtraBytes.size());
                        break;
                    } else {//需要组包
//                        mBytes.add(temp);
                        Log.e(TAG, "less: " + mBytes.size());
                        mIsEnough = false;
                    }
                }
            }
//            if (len == -1) {
            if (!mIsEnough) {
//                Log.e(TAG, "receive len error: " + len);
                return null;
            }
        } catch (IOException e) {
            e.printStackTrace();
        ...
        Log.e(TAG, "final return: " + mBytes.size());
        return mBytes;
    ...
    public void setOnSnippetGetListener(TcpMsgStickyResultListener listener) {
        mListener = listener;
    }

    interface TcpMsgStickyResultListener {
        void onSnippetGet(List<Byte> bytes);
    }
}
